...
function lightning_get_common_inline_css() {
	...
	if ( $max_height_pc ) {
		$dynamic_css .= '@media ( 992px < width ) { .site-header-logo img { max-height: ' . $max_height_pc . 'px; } }';
	}
	if ( $max_height_mobile ) {
		$dynamic_css .= '@media ( width <= 992px ) { .site-header-logo img { max-height: ' . $max_height_mobile . 'px; } }';
	}
