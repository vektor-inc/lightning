...
function lightning_load_common_editor_css_to_gutenberg() {
	...
	wp_enqueue_style( 'lightning-common-editor-gutenberg', get_template_directory_uri() . '/assets/css/common_editor.css', array( 'wp-edit-blocks' ), LIGHTNING_THEME_VERSION );
}
...
if ( ! function_exists( 'lightning_widgets_init' ) ) {
	...
	function lightning_widgets_init() {
		...
		// Sidebar( post_type ).
		$the_post_types = get_post_types( array( 'public' => true ) );

		foreach ( $the_post_types as $the_post_type ) {

			// Get post type name.
			$post_type_object = get_post_type_object( $the_post_type );
			if ( $post_type_object ) {
				// Set post type name.
				$the_post_type_name = esc_html( $post_type_object->labels->name );
				...
				if ( 'post' === $the_post_type ) {
					...
					// translators: The sidebar description of custom posy type.
					$sidebar_description = sprintf( __( 'This widget area appears on the %s contents page only.', 'lightning' ), $the_post_type_name );
				...
				register_sidebar(
					array(
						// translators: The sidebar name of custom posy type.
						'name'          => sprintf( __( 'Sidebar(%s)', 'lightning' ), $the_post_type_name ),
						'id'            => $the_post_type . '-side-widget-area',
...
/**
 * Disable Tgm Notification Except Admin
 */
