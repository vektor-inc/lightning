<?php
/**
 * Next Prev Template of Lightning
 *
 * @package Lightning
 */
...
$the_taxonomy   = apply_filters( 'lightning_prev_next_post_taxonomy', 'category' );
if ( '3' === $bootstrap ) {
	?>
	<nav>
		<ul class="pager">
		<li class="previous"><?php previous_post_link( '%link', '%title', $in_same_term, $excluded_terms, $the_taxonomy ); ?></li>
		<li class="next"><?php next_post_link( '%link', '%title', $in_same_term, $excluded_terms, $the_taxonomy ); ?></li>
		</ul>
	</nav>
	<?php
}
if ( '4' === $bootstrap ) {
	$post_previous = get_previous_post( $in_same_term, $excluded_terms, $the_taxonomy );
	$post_next     = get_next_post( $in_same_term, $excluded_terms, $the_taxonomy );
