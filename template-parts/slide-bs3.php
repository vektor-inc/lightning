...
if ( apply_filters( 'lightning_default_slide_display', true ) ) {
	...
	if ( $top_slide_count ) : ?>
		...
		?>
	<div id="top__fullcarousel" data-interval="<?php echo esc_html( $interval ); ?>" class="carousel slide slide-main" data-ride="carousel">
		<div class="carousel-inner">
			...
			<?php if ( $top_slide_count >= 2 ) : ?>
				...
				<ol class="carousel-indicators">
					...
					<li data-target="#top__fullcarousel" data-slide-to="<?php echo esc_html( $i ); ?>"></li>
						<?php
						$i++;
					}
					?>
			...
			for ( $i = 1; $i <= $top_slide_count_max; ) {
				...
				if ( ! empty( $lightning_theme_options[ 'top_slide_image_' . $i ] ) ) {
					...
					?>
					<div class="item item-<?php echo esc_html( $i ); ?><?php echo esc_html( $active ); ?>">
						...
						<div class="slide-text-set mini-content <?php echo esc_attr( $mini_content_args['style_class'] ); ?>"<?php echo $style; ?>>
							<div class="container">
								...
								if ( $mini_content_args['link_url'] && $mini_content_args['btn_text'] ) :
									...
									echo '.' . esc_attr( $style_class ) . ' .btn-ghost { border-color:' . esc_attr( $mini_content_args['text_color'] ) . ';color:' . esc_attr( $mini_content_args['text_color'] ) . ';' . $box_shadow . $text_shadow . ' }';
									echo '.' . esc_attr( $style_class ) . ' .btn-ghost:hover { border-color:' . esc_attr( $mini_content_args['btn_bg_color'] ) . '; background-color:' . esc_attr( $mini_content_args['btn_bg_color'] ) . '; color:#fff; text-shadow:none; }';
									echo '</style>';
									?>
