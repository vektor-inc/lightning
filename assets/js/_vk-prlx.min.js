...
(function(window, document, $) {
    window.addEventListener('DOMContentLoaded', prlx_scr)
    window.addEventListener('scroll', prlx_scr)
    window.addEventListener('resize', prlx_scr)

    function prlx_scr() {
        var scrled_Y = $(window).scrollTop();
        var window_H = document.documentElement.clientHeight;
        var urlRegex = /.*url\(['"]*(.*?)['"]*\).*/g;
        $('.vk-prlx').each(function(i) {
            var url = $(this).css('background-image').replace(urlRegex, '$1');
            if (url != 'none') {
                var img = new Image();
                img.src = url;
                var img_W = img.width;
