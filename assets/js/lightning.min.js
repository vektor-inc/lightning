...
jQuery(this).css({"background-position":backgroundPosition,"background-size":bg_W+'px '+bg_H+'px',});}});}})(jQuery);;(function($){function vk_mobile_nav_close(target){jQuery(target).each(function(){jQuery(this).removeClass('menu-open');});jQuery('.mobile-fix-nav .vk-mobile-nav-menu-btn i').removeClass('fa-times');jQuery('.mobile-fix-nav .vk-mobile-nav-menu-btn i').addClass('fa-bars');}
...
console.log('_|＼○_ﾋｬｯ ε=＼＿○ﾉ ﾎｰｳ!!');jQuery(document).ready(function(){vk_mobile_nav_run('.vk-mobile-nav-menu-btn');});})(jQuery);;(function($){function vk_menu_acc_run(){var breakPoint=5000;var bodyWidth=window.innerWidth;if(bodyWidth<=breakPoint){$.when(vk_menu_acc_clear()).done(function(){vk_menu_acc_init();vk_menu_acc_click();});}else{vk_menu_acc_clear();}}
